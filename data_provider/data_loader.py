...
class Dataset_Custom_orig(Dataset):
    """
...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        '''
        df_raw.columns: |'date', ---- (other features) ------, 'OT'|
        '''
        # 強制數據順序，將時間放到的第一個，OT放到最後
        cols = df_raw.columns.tolist()
        cols.remove(self.target)  # self.target='OT'
        cols.remove('date')
        df_raw = df_raw[['date'] + cols + [self.target]]
...
def custom_collate_fn(feature, data):
    """
    注意在生产环境是，OT是最后一列，要加上
    data = [(seq_x, seq_y, seq_x_mark, seq_y_mark), ....], 共batch_size个
    """
    x_scaled_batch, x_scalers, y_scaled_batch, y_scaler = [], [], [], []
    x_mark_batch, y_mark_batch = [], []

    for i, (seq_x, seq_y, seq_x_mark, seq_y_mark) in enumerate(data):
        # x: (768, 26), y:(96, 26), x_m:(768, 5), y_m:(96, 5)
        # print(f"i: {i}, x: {seq_x.shape}, y:{seq_y.shape}, x_m:{seq_x_mark.shape}, y_m:{seq_y_mark.shape}")
        x_mark_batch.append(seq_x_mark)
        y_mark_batch.append(seq_y_mark)

        scaler = StandardScaler()
        scaled_sample = scaler.fit_transform(seq_x)
        x_scaled_batch.append(scaled_sample)
        x_scalers.append(scaler)

        if feature == "S" or feature == "MS":
            # 只预测OT（数据的最后一列）时，
            """
            seq_y.shape should be (96,1)
            """
            x_target = seq_x[:, -1]
            target_scaler = StandardScaler()
            target_scaler.fit(x_target.reshape(-1, 1))
            y_target = seq_y[:, -1]
            scaled_sample = target_scaler.transform(y_target.reshape(-1, 1))
            y_scaled_batch.append(scaled_sample)
            y_scaler.append(target_scaler)

        elif feature == "MM":
            """
            seq_y.shape should be (96,26)
            """
            # share scaler with seq_x
            scaled_sample = scaler.fit_transform(seq_y)
            y_scaled_batch.append(scaled_sample)
            y_scaler.append(scaler)

    seq_x = torch.stack([torch.from_numpy(arr) for arr in x_scaled_batch])
    seq_y = torch.stack([torch.from_numpy(arr) for arr in y_scaled_batch])
    x_mark = torch.stack([torch.from_numpy(arr) for arr in x_mark_batch])
    y_mark = torch.stack([torch.from_numpy(arr) for arr in y_mark_batch])
    # print(f"seq_x: {seq_x.shape}, seq_y: {seq_y.shape}, x_mark:{x_mark.shape}, y_mark:{y_mark.shape}")
